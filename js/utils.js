...
const Utils = (() => {
    ...
    // Format: SCK/001/MM/YYYY (auto-increment per bulan)
    async function generateNoPenjualan() {
        const now = new Date();
        ...
        // Get existing penjualan to find max number this month
        const allSales = await DB.getAll('penjualan');
        let maxNum = 0;
        const regex = new RegExp(`^SCK/(\\d+)/${mm}/${yyyy}$`);
        for (const sale of allSales) {
            if (sale.noPenjualan) {
                const match = sale.noPenjualan.match(regex);
                if (match) {
    ...
    return {
        ...
        formatNumber, generateId, generateNoPenjualan,
