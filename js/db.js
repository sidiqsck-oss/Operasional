...
const DB = (() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (e) => {
                ...
                if (!database.objectStoreNames.contains('penjualan')) {
                    ...
                    store.createIndex('noPenjualan', 'noPenjualan', { unique: false });
                    store.createIndex('statusJual', 'statusJual', { unique: false });
                    store.createIndex('shipment', 'shipment', { unique: false });
                }
    ...
    async function removeMaster(type, value) {
        return remove('master_data', [type, value]);
    }

    // --- Full Export/Import for backup ---
    async function exportAll() {
        const stores = ['induksi', 'reweight', 'penjualan', 'master_data', 'users', 'settings', 'logs'];
        const backup = { _version: DB_VERSION, _date: new Date().toISOString(), _app: 'FeedlotOps5' };
